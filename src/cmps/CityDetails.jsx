...
export function CityDetails() {
    ...
    const checkFavorite = () => {
        if (activeCity) {
            ...
            const isFavorite = list?.find(favorite => favorite.key === activeCity.key)
    ...
    return (
        <div className={`city-details flex column ${isLight ? 'bgcLight' : 'bgcDark'}`}>
            <div className="flex space-between">
                <div className="left-top flex">

                    <Map className="map" activeCity={activeCity} />
                    <div className="flex column">
                        <span className={`name ${isLight ? 'light' : 'dark'}`}>{activeCity.name}</span>
                        <span className={`celsius ${isLight ? 'light' : 'dark'}`}
                            onClick={() => toggleCelsius()}>{
                                temperature ?
                                    getCelsius() + '°C' :
                                    activeCity?.days[0].Temperature.Maximum.Value + 'F'}</span>
                        <span className={` ${isLight ? 'light' : 'dark'}`}>{activeCity.TimeZone}</span>

                    </div>
                </div>
                <div className="right-top flex">
                    <span className={`btn-favorite ${isFavorite ? 'favorite' : ''}`} onClick={() => onSetFavorite()}><BookmarkBorderIcon /></span>
                </div>
            </div>
            <div className="day-title flex column ">
                <img className="img" src={img} alt="img" />

                <h2 className={`phrase ${isLight ? 'light' : 'dark'}`}>{activeCity?.days[0].Day.IconPhrase}</h2>
            </div>
