...
async function getCityKey(value) {
    ...
    if (!city) {
        const res = await axios.get(`http://dataservice.accuweather.com/locations/v1/cities/search?apikey=${WEATHER_KEY}&q=${value}`)
...
async function queryDaysWeather(keyCity) {
    const res = await axios.get(`http://dataservice.accuweather.com/forecasts/v1/daily/5day/${keyCity}?apikey=${WEATHER_KEY}`);
...
async function callApiOptions(debounceSearchTrm) {
    const res = await axios.get(`http://dataservice.accuweather.com/locations/v1/cities/autocomplete?apikey=${WEATHER_KEY}&q=${debounceSearchTrm}`)
